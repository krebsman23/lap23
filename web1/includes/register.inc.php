...
if(isset($_POST['register-submit'])) {
  ...
  if( empty($username) || empty($password) || empty($password_confirm)){
      ...
      $sql = "INSERT INTO users (user, password,  email, postal_code, street, place, course_id) VALUES (?,?,?,?,?,?,?)";
      $statement = $db->prepare($sql);
      $statement->bind_param("sssssss", $username, $password, $email, $postal_code, $street, $place, $register_course_id);
      $statement->execute();
      $statement->close();
